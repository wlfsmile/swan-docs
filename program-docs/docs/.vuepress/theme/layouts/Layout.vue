...
export default {
    ...
    mounted() {
        ...
        this.headerNavIndex = getInitHeaderIndex(this.headerNav, currentPageHeader);
        if (this.subNavIsShow) {
            this.subNavIndex = getInitSubIndex(this.subNav, this.$route.path);
        }
    ...
    computed: {
        ...
        subNav() {
            return this.navData[this.headerNavIndex].nav.map(item => {
                ...
                return {text: item.text, link, close, sidebar: item.sidebar};
            });
        },
        // 侧导航
        sidebarNav() {
